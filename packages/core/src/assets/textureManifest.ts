...
/**
 * Extended wall piece types for modular wall rendering (Phase 7).
 * Maps to piece naming conventions in pack manifests.
 */
...
/** Map from WallPieceType to pack manifest material name patterns */
const PIECE_TYPE_PATTERNS: Record<WallPieceType, string[]> = {
  'straight': ['Straight'],
  'end-cap': ['Ending'],
  'inner-corner': ['Corner'],
  'outer-corner': ['Corner'],
  't-junction': ['Joint'],
  'cross': ['Connector'],
  'doorway': ['Connector_DIAG', 'Connector'],
  'window': ['Connector'],
};
...
/**
 * Resolve a modular wall piece from a pack manifest by piece type.
 * Searches pack entries for material names matching the piece type pattern.
 *
 * @param packEntryIds - All entry IDs for the pack (e.g. 'dungeon-classic:Fence_Stone_Slate_A_Corner_A_1x1_1x1_wall_A')
 * @param wallSetPrefix - Material family prefix (e.g. 'Fence_Stone_Slate_A')
 * @param pieceType - The modular piece type to find
 * @returns Matching entry IDs sorted by grid size (smallest first)
 */
export function resolvePackWallPiece(
  packEntryIds: string[],
  wallSetPrefix: string,
  pieceType: WallPieceType,
): string[] {
  const patterns = PIECE_TYPE_PATTERNS[pieceType];
  return packEntryIds.filter((id) => {
    const localId = id.includes(':') ? id.split(':')[1] : id;
    if (!localId) return false;
    // Entry must belong to the same wall set family
    if (!localId.startsWith(wallSetPrefix)) return false;
    // Check if any pattern matches the material portion
    return patterns.some((p) => localId.includes(`_${p}_`) || localId.includes(`_${p}`));
  });
}
